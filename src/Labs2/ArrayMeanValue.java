...
public class ArrayMeanValue {
    ...
    public static int brojDoProsek(Array<Integer> arr) {
        double average = findProsek(arr);
        int arraySize = arr.getSize();
        double tmp;
        double min = 999999;
        int resultIndex = 0;
        for (int i = 0; i < arraySize; i++) {
            tmp = Math.abs(average - arr.get(i));

            if (tmp < min) {
                min = tmp;
                resultIndex = i;
            }
            if (tmp == min) { // if 2 separate numbers have same difference, get smaller
                if (arr.get(i) < arr.get(resultIndex)) {
                    min = tmp;
                    resultIndex = i;
                }
            }
        }
        return arr.get(resultIndex);
    }

    private static double findProsek(Array<Integer> arr) {
        double sum = 0;
        int sizeArray = arr.getSize();
        for (int i = 0; i < sizeArray; i++) {
            sum += arr.get(i);
        }
        return sum / (double) sizeArray;
    }
