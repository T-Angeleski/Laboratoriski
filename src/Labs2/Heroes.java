...
public class Heroes {
    ...
    public static void startHeroesGame(SLL<Card> firstFriendCards,
                                       SLL<Card> secondFriendCards) {
        //Find the strongest card from the first player
        SLLNode<Card> tmp = firstFriendCards.getFirst();
        Card strongest = new Card(0, 0, 0); //default card
        int max = tmp.element.damage();
        while (tmp != null) {
            if (tmp.element.damage() > max) {
                max = tmp.element.damage();
                strongest = tmp.element;
            }
            tmp = tmp.succ;
        }
        tmp = firstFriendCards.find(strongest);
        firstFriendCards.delete(tmp);

        //Add the strongest card to second friend
        SLLNode<Card> tmp2 = secondFriendCards.getFirst();
        int counter = 1;
        while (tmp2 != null) {
            //when counter is at 3 we know we're at middle
            if (counter == 3)
                secondFriendCards.insertAfter(strongest, tmp2);
            counter++;
            tmp2 = tmp2.succ;
        }
    }
