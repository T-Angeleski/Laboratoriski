...
class BinarySearchTree<E extends Comparable<E>> {
	...
	public void maxCustomers() {
		maxCustomersRecursive(root, 0);
	}

	private int maxCustomersRecursive(BNode<E> node, int b) {
		if (node == null) {
			return 0;
		}

		if (node.left != null) {
			b = maxCustomersRecursive(node.left, b);
		}
		...
		if (node.right != null) {
			b = maxCustomersRecursive(node.right, b);
		}
